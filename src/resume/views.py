...
from src.resume.service import (
    get_all_resume,
    insert_resume,
    update_resume,
    delete_resume,
...
async def create_resume(
    resume: ResumeIn, session: AsyncSession = Depends(tempest.async_scoped_session)
):
    return await insert_resume(resume=resume, session=session)


@router.post("/change_resume")
async def change_resume(
    resume: ResumeIn, session: AsyncSession = Depends(tempest.async_scoped_session)
):
    return await update_resume(resume=resume, session=session)
...
async def drop_resume(
    ...
    return await delete_resume(resume=resume, session=session)
